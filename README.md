...
| The diff of that MR, with folds and syntax colours | `]c` next change, `]n` next open thread, `T` every thread, `za` fold, `D` side by side, `c` comment on a line, drag to copy the code |
