...
impl std::fmt::Debug for TypeSafe {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("TypeSafe").field("url", &self.url).field("key", &"<redacted>").finish_non_exhaustive()
    }
}

impl TypeSafe {
    /// Fails fast when TypeSafe is switched off by the environment, before anything is sent.
    pub fn connect(key: &super::Secret) -> Result<Self, Unavailable> {
        if let Some(reason) = disabled_reason() {
            return Err(Unavailable(reason.into()));
        }
        Self::new(API_URL, key.expose())
    }
...
fn classify_failure(status: u16, message: &str) -> Failure {
    ...
    if status == 401 {
        return Failure::Final(Unavailable("API key rejected (HTTP 401): run `revu ai login typesafe`".into()));
    }
