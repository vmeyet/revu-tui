//! How a merge request is named on the command line: `group/project!42`, `!42`, an MR URL, or nothing.
...
pub fn parse(text: &str, local_project: impl FnOnce() -> Option<String>) -> Result<MrRef> {
    ...
    let (project, iid) = text.rsplit_once('!').with_context(|| format!("not an MR reference: {text} (try group/project!42)"))?;
    let iid: u64 = iid.parse().with_context(|| format!("not an MR number: {iid}"))?;
    let project = match project {
        "" => ProjectRef::Path(local_project().context("`!42` needs a GitLab checkout with an origin remote")?),
...
mod tests {
    ...
    fn garbage_says_what_a_reference_looks_like() {
        for text in ["nonsense", "acme/widgets!x", "https://gitlab.com/acme/widgets/-/issues/1", "!"] {
