...
Row 2 ends with the review app, once the branch was deployed: `⧉ review/feat-x +1` in the link colour, clickable, the app to try (a `review/` environment, else the first) and how many more the pipeline pane lists; `muted` with `· older push` when it was deployed from an earlier push (GitLab: not the MR's newest pipeline, as merge request pipelines deploy a merge commit; GitHub: not the head commit).
