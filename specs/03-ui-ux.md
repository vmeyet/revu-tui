...
Row 2 ends with the review app, once the branch was deployed: `⧉ review/feat-x +1` in the link colour, clickable, the first environment and how many more the pipeline pane lists; `muted` with `· older commit` when it runs a commit older than the head.
